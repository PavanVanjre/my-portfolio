    ...
    <section id="home" className="min-h-screen flex items-center justify-center relative overflow-hidden px-4">
      <div className="container mx-auto ">
        ...
        >
          ...
          <motion.div variants={itemVariants} className="mb-12 h-8 text-xl md:text-2xl text-muted-foreground">
            <span>{heroData.title} </span>
            <TypeAnimation
              sequence={heroData.roles.flatMap(role => [role, 2000])}
              wrapper="span"
          ...
          >
            ...
            <div className="space-y-6">
              {aboutData.content.map((paragraph, index) => (
                <p key={index} className="text-lg text-muted-foreground leading-relaxed text-left">
                  {paragraph}
                </p>
              ))}
            </div>
              ...
              {heroData.socialLinks.map((link, index) => {
                ...
                return (
                  <motion.a
                    ...
                    whileTap={{ scale: 0.9 }}
                  >
                    <IconComponent size={40} className="text-primary/80 hover:text-primary" />
                  </motion.a>
                );
              })}
            </div>
